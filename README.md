...
`GET /agents` should return `{ "agents": [...] }`. Each agent can include `id`, `name`, `type`, `status`, `task`, `currentStep`, `progressPercent`, `owner`, `workspace`, `repository`, `branch`, `queue`, `priority`, `cpu`, `memoryMb`, `processCpu`, `processMemoryMb`, `childCpu`, `childMemoryMb`, `tokens`, `tokensPerSecond`, `tokenRateWindowMs`, `tokenCountConfidence`, `costUsd`, `startedAt`, `endedAt`, `parentId`, `children`, `pid`, `parentPid`, `childPids`, `goToTarget`, `goToKind`, and `windowTitle`. Remote adapters preserve the process-resource breakdown fields when providers report them. Agent Monitor normalizes resource fields and `costUsd` to numbers before rendering meters, totals, or sorted task lists, accepts numeric or parseable date-string timestamps for agent/log/transcript times, and normalizes `parentId` plus `children` entries to string IDs for stable lineage joins.
