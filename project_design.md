...
Local process metadata is intentionally best-effort. Configured agents can provide a short description and context-window total; discovered agents infer short descriptions from visible CLI flags such as `--description`, `--title`, `--task`, or `--prompt`. These fields flow into `shortDescription`, `terminalTitle`, `contextWindowUsed`, `contextWindowTotal`, `contextWindowConfidence`, and `thinkingSnippet`. The browser detail panel renders the values directly, while the Three.js Office focused cubicle renders them as a board meter and pinned notes.
