...
Local process metadata is intentionally best-effort, but known local CLIs can have provider-specific enrichment. Configured agents can provide a short description and context-window total; discovered agents infer short descriptions from visible CLI flags such as `--description`, `--title`, `--task`, or `--prompt`; discovered OpenCode agents read the local SQLite session store at `~/.local/share/opencode/opencode.db` for the latest session title, step token total, cumulative token count, reasoning snippet, and todo/tool status. These fields flow into `shortDescription`, `terminalTitle`, `contextWindowUsed`, `contextWindowTotal`, `contextWindowConfidence`, and `thinkingSnippet`. The browser detail panel uses the local/session title as the primary agent title and renders context-window/thinking cards, while the Three.js Office focused cubicle renders them as a board meter and pinned notes.
